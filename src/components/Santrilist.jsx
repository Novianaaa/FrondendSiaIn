...
useEffect(()=>{
    ...
    const getSantri = async () => {
        const response = await axios.get(`http://localhost:5000/santri?search_query=${keyword}&page=${page}&limit=${limit}`);
        setSantri(response.data.result);
    ...
    const deleteSantri =async (userId)=>{
        await axios.delete(`http://localhost:5000/santri/${userId}`);
        getSantri();
    }
  ...
  return (
    <div>
        ...
        <h2 className="subtitle">List of Data Santri</h2>
        {/* <Link to="/santri/add" className='button is-primary mb-2'>Add Santri</Link> */}
                        ...
                        <table className='table is-striped is-fullwidth mt-2'>
                            ...
                            {santri.map((santri, index) => (
                                <tr key={santri.uuid}>
                                    ...
                                    <td>{santri.nis}</td>
                                    <td>{santri.noTelepon}</td>
                                    <td>{santri.tanggalLahir}</td>
                                    <td>{santri.namaWali}</td>
                                    <td>
                                        <Link to={`/santri/detail/${santri.uuid}`} className="button is-small is-info mr-2"><AiOutlineFileSearch className='mr-2'/>Detail</Link>
